...
var figureCode = {
  ...
  rectangle: function(elm, canvWidth) {
    return elm.rect(canvWidth / 5, canvWidth / 2.5, canvWidth / 5 * 3, canvWidth / 5);
  },
  ...
  poly5: function(elm, canvWidth) {
    var x = canvWidth / 2, y = canvWidth / 2, r = canvWidth / 4, n = 5;
  ...
  poly6: function(elm, canvWidth) {
    var x = canvWidth / 2, y = canvWidth / 2, r = canvWidth / 4, n = 6;
    var xx, yy, i, a, pathString = '';
    for (i = 0; i <= n; ++i) {
        a = ((4 * Math.PI * i) + (Math.PI * n) + (2 * Math.PI)) / (2 * n);
        xx = x + r * Math.cos(a);
