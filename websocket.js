...
function handleConnection(ws) {
  ...
  if (token) {
    ...
    // support resetting task count, for a "reset count and start timer" - button
    ws.on('reset-daily-task-count', (msg) => {
      console.log('reset-daily-task-count', msg);
      if (decoded.classId === msg.classId) {
        return config.getDatabaseClient().then((client) => {
          return client
            .query(
